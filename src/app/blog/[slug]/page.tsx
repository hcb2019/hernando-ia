...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  if (!post) return { title: 'Post Not Found | Hernando.ia' }
  ...
  return (
    <div className="min-h-screen bg-[#08081a]">
      ...
      <main className="relative max-w-3xl mx-auto px-4 sm:px-8 py-24">
        ...
        <Link
          href="/blog"
          className="inline-flex items-center gap-2 text-sm text-white/40 hover:text-accent transition-colors mb-10"
        >
          ← Back to blog
        </Link>
        ...
        <header className="mb-12">
          ...
          <div className="flex items-center gap-3 text-sm text-white/40">
            ...
            {post.sponsored && (
              <>
                <span className="text-white/20">·</span>
                <span className="text-accent/70 font-medium">Sponsored</span>
              </>
            )}
            {post.premium && (
              <>
        ...
        {post.premium && (
          <div className="glass p-6 mb-10 glow-border text-center">
            ...
            <p className="text-white/50 text-sm mb-4">
              This is a premium article. Subscribe to unlock the full content.
            </p>
        ...
        {/* Monetization: Sponsor disclosure */}
        {post.sponsored && (
          <div className="mt-12 glass p-4 text-center">
            <p className="text-xs text-white/30">
              This post contains sponsored or affiliate content.{' '}
              <a href="/sponsorship" className="text-accent/50 hover:text-accent transition-colors">
                Learn more about our sponsorship policy.
              </a>
            </p>
          </div>
        )}
        ...
        <div className="mt-16 glass p-8 text-center glow-border">
          ...
          <p className="text-white/50 text-sm mb-5">
            Subscribe to the newsletter for more AI engineering insights delivered
...
function renderMarkdown(md: string): string {
  ...
  // Headings
  html = html.replace(/^#### (.+)$/gm, '<h4 class="text-lg font-semibold mt-8 mb-3 text-white/80">$1</h4>')
  html = html.replace(/^### (.+)$/gm, '<h3 class="text-xl font-medium mt-10 mb-4 text-white/80">$1</h3>')
  html = html.replace(/^## (.+)$/gm, '<h2 class="text-2xl font-semibold mt-12 mb-5 text-white/90">$1</h2>')
  html = html.replace(/^# (.+)$/gm, '<h1 class="text-4xl font-bold mt-14 mb-6 glow-text">$1</h1>')
  ...
  // Inline code
  html = html.replace(/`([^`]+)`/g, '<code class="bg-surface px-1.5 py-0.5 rounded text-sm font-mono text-accent">$1</code>')

  // Links
  html = html.replace(/\[([^\]]+)\]\(([^)]+)\)/g, '<a href="$2" class="text-accent hover:underline decoration-accent/50 underline-offset-4">$1</a>')

  // Images
  html = html.replace(/!\[([^\]]*)\]\(([^)]+)\)/g, '<img src="$2" alt="$1" class="rounded-lg my-8 max-w-full" />')

  // Horizontal rules
  html = html.replace(/^---$/gm, '<hr class="my-10 border-border" />')

  // Blockquotes
  html = html.replace(/^> (.+)$/gm, '<blockquote class="border-l-4 border-accent/50 pl-4 my-6 italic text-white/60">$1</blockquote>')

  // Unordered lists — handle blocks of consecutive list items
  html = html.replace(/((?:^- .+\n?)+)/gm, (match) => {
    const items = match
      .split('\n')
